...
public:
    ...
    const Type* byRva(const Rva rva) const noexcept
    {
        ...
        constexpr auto k_minimalSectionAlignment = 16u;
...
public:
    ...
    {        
        ...
        using value_type = GenericTypes::SecHeader;
        using difference_type = GenericTypes::SecHeader;
        using pointer = GenericTypes::SecHeader*;
        using reference = GenericTypes::SecHeader&;
...
public:
    ...
    public:
        ...
        const GenericTypes::ImgImportByName* name() const noexcept
        {
            ...
            return m_lib.pe().template byRva<GenericTypes::ImgImportByName>(rva);
        }
...
public:
    ...
    public:
        ...
        const typename Types<arch>::ImportAddressTableEntry* importAddressTable() const noexcept
        {
            return m_pe.template byRva<Types<arch>::ImportAddressTableEntry>(m_descriptor->ImportAddressTableRVA);
        }

        // Import Name Table:
        const typename Types<arch>::ImportNameTableEntry* importNameTable() const noexcept
        {
            return m_pe.template byRva<Types<arch>::ImportNameTableEntry>(m_descriptor->ImportNameTableRVA);
        }
...
public:
    explicit Exports(const Pe<arch>& pe) noexcept
        ...
        , m_descriptor(m_directory ? pe.template byRva<DirExports::Type>(m_directory->VirtualAddress) : nullptr)
        , m_tables(m_descriptor
            ? Tables
              {
                  pe.template byRva<GenericTypes::ExportAddressTableEntry>(m_descriptor->AddressOfFunctions),
...
public:
    ...
    public:
        ...
        typename GenericTypes::FnImageTlsCallback callback() const noexcept
        {
            ...
            return reinterpret_cast<GenericTypes::FnImageTlsCallback>(m_tls.pe().template byRva<void>(rva));
        }

        typename GenericTypes::FnImageTlsCallback callback_mapped() const noexcept
        {
            return static_cast<GenericTypes::FnImageTlsCallback>(*m_callbackPointer);
        }
...
public:
    ...
    const typename GenericTypes::FnImageTlsCallback* callbacks() const noexcept
    {
        ...
        case ImgType::file:
        {
            return m_pe.template byRva<GenericTypes::FnImageTlsCallback>(static_cast<Rva>(m_directory.ptr->AddressOfCallBacks - m_pe.imageBase()));
        }
        case ImgType::module:
        {
            return reinterpret_cast<GenericTypes::FnImageTlsCallback*>(m_directory.ptr->AddressOfCallBacks);
        }
        }
